            ...
            return (
              <div className={`absolute z-60 ${dropdownClassName} ${positionClass}`}>
                <CustomDropdown mode="dark">
                  <div className={`${bgClassName} p-2`} style={{ backgroundColor: 'var(--popup-bg, #FFFFFF)' }}> 
                    <h4 className="font-semibold mb-2" style={{ color: 'var(--popup-text, #181D27)' }}>Notifications</h4>
                    <div className="flex flex-col gap-2 max-h-64 overflow-auto">
                      {items.length === 0 && (
                        <div className="px-2 py-2 text-sm" style={{ color: 'var(--popup-secondary-text, #535862)' }}>No notifications</div>
                      )}
                      {items.map((it) => (
                        <div
                          key={it.id || it.title}
                          className="px-2 py-2 rounded cursor-pointer"
                          style={{ backgroundColor: 'var(--dropdown-hover-bg, #F3F4F6)' }}
                          onClick={() => {
                            it.onClick && it.onClick();
                            setOpen(false);
                          }}
                        >
                          <div className="font-medium text-sm" style={{ color: 'var(--popup-text, #181D27)' }}>{it.title}</div>
                          {it.description && <div className="text-xs" style={{ color: 'var(--popup-secondary-text, #6B7280)' }}>{it.description}</div>}
                        </div>
                      ))}
                    </div>
