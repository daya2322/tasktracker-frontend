...
interface AdminActivity { id: number; action: string; target: string; time: string; type: "company" | "system" | "billing" | "security"; }
...
// ─── SVG Bar Chart ─────────────────────────────────────────────────────────────
function BarChart({ data, color, h = 80 }: { data: { label: string; value: number }[]; color: string; h?: number }) {
  const tk = useT();
...
export default function AdminDashboard() {
  ...
  const totalCompanies = useCountUp(47);
  const totalRevenue   = useCountUp(284600);
  const totalUsers     = useCountUp(4812);
  const activeSessions = useCountUp(312);
  ...
  const [formData, setFormData] = useState<CompanyFormData>({ name: "", industry: "", owner: "", email: "", phone: "", plan: "Starter", employees: "" });

  const INDUSTRIES = ["Technology", "Finance", "Healthcare", "Retail", "Education", "Manufacturing", "Media", "Logistics", "Real Estate", "Other"];

  const [companies, setCompanies] = useState<Company[]>([
    { id:1, name:"TechNova Solutions",  industry:"Technology",    plan:"Enterprise", employees:320, status:"active",    revenue:48000, joinDate:"Jan 2024", owner:"Amit Desai",     email:"amit@technova.com",  avatar:"TN" },
    { id:2, name:"FinEdge Corp",        industry:"Finance",       plan:"Pro",        employees:145, status:"active",    revenue:22000, joinDate:"Mar 2024", owner:"Priya Mehta",    email:"priya@finedge.com",  avatar:"FE" },
    { id:3, name:"HealthFirst Ltd",     industry:"Healthcare",    plan:"Pro",        employees:89,  status:"trial",     revenue:8000,  joinDate:"Feb 2025", owner:"Dr. Suresh Kumar", email:"sk@healthfirst.com", avatar:"HF" },
    { id:4, name:"RetailMax Inc",       industry:"Retail",        plan:"Starter",    employees:56,  status:"active",    revenue:6400,  joinDate:"Nov 2024", owner:"Kavita Rao",     email:"krao@retailmax.com", avatar:"RM" },
    { id:5, name:"EduSpark Academy",    industry:"Education",     plan:"Starter",    employees:34,  status:"suspended", revenue:0,     joinDate:"Aug 2024", owner:"Rajan Pillai",   email:"rajan@eduspark.com", avatar:"ES" },
    { id:6, name:"LogiRoute Systems",   industry:"Logistics",     plan:"Enterprise", employees:210, status:"active",    revenue:31000, joinDate:"Jun 2023", owner:"Mohan Tiwari",   email:"mt@logiroute.com",   avatar:"LR" },
    { id:7, name:"MediaPulse Group",    industry:"Media",         plan:"Pro",        employees:78,  status:"active",    revenue:14500, joinDate:"Apr 2024", owner:"Aisha Khan",     email:"ak@mediapulse.com",  avatar:"MP" },
    { id:8, name:"BuildSmart Realty",   industry:"Real Estate",   plan:"Starter",    employees:42,  status:"trial",     revenue:3200,  joinDate:"Jan 2025", owner:"Dinesh Sharma",  email:"ds@buildsmart.com",  avatar:"BS" },
  ]);

  const activities: AdminActivity[] = [
    { id:1, action:"New company registered",     target:"TechNova Solutions",  time:"5m ago",   type:"company"  },
    { id:2, action:"Plan upgraded to Enterprise", target:"LogiRoute Systems",   time:"22m ago",  type:"billing"  },
    { id:3, action:"Suspicious login detected",   target:"IP: 192.168.4.21",    time:"1h ago",   type:"security" },
    { id:4, action:"Company suspended",           target:"EduSpark Academy",    time:"2h ago",   type:"company"  },
    { id:5, action:"Server CPU spike (94%)",       target:"us-east-1 node",      time:"3h ago",   type:"system"   },
    { id:6, action:"Invoice generated",           target:"FinEdge Corp - ₹22k", time:"4h ago",   type:"billing"  },
    { id:7, action:"New admin created",           target:"ops@worksphere.io",   time:"5h ago",   type:"security" },
    { id:8, action:"Database backup completed",   target:"prod-db-snapshot",    time:"6h ago",   type:"system"   },
  ];
  ...
  const revenueData = [
    { label:"Sep", value:18400 }, { label:"Oct", value:22100 }, { label:"Nov", value:19800 },
    { label:"Dec", value:26400 }, { label:"Jan", value:24200 }, { label:"Feb", value:28900 },
    { label:"Mar", value:31200 }, { label:"Apr", value:29600 }, { label:"May", value:33800 },
    { label:"Jun", value:36100 }, { label:"Jul", value:38400 }, { label:"Aug", value:41200 },
  ];

  const planDistribution = [
    { v:12, color:"#f59e0b", label:"Enterprise" },
    { v:21, color:"#8b5cf6", label:"Pro"        },
    { v:14, color:"#64748b", label:"Starter"    },
  ];

  const filteredCompanies = useMemo(() => {
    return companies.filter(c => {
      const matchSearch = c.name.toLowerCase().includes(companySearch.toLowerCase()) ||
        c.owner.toLowerCase().includes(companySearch.toLowerCase()) ||
  ...
  const addCompany = () => {
    ...
    setFormError("");
    const initials = formData.name.trim().split(" ").map(w => w[0]).slice(0, 2).join("").toUpperCase();
    const newCompany: Company = {
      id: Date.now(), name: formData.name.trim(), industry: formData.industry || "Technology",
      plan: formData.plan, employees: parseInt(formData.employees) || 0,
      status: "trial", revenue: 0, joinDate: new Date().toLocaleDateString("en-US", { month: "short", year: "numeric" }),
      owner: formData.owner.trim(), email: formData.email.trim(), avatar: initials,
    };
    setCompanies(prev => [...prev, newCompany]);
    setFormData({ name: "", industry: "", owner: "", email: "", phone: "", plan: "Starter", employees: "" });
    setShowAddModal(false);
    setActiveTab("companies");
  };

  const toggleSuspend = (id: number) => {
    setCompanies(prev => prev.map(c => c.id === id ? { ...c, status: c.status === "suspended" ? "active" : "suspended" } : c));
    ...
    <>
      ...
      <div className="ad-root" style={{ background: tk.bg, minHeight: "100%", padding: "24px 24px 36px", display: "flex", flexDirection: "column", gap: 22, transition: "background 0.3s" }}>
        ...
        <div className="ad-a2" style={{ display: "grid", gridTemplateColumns: "repeat(4,minmax(0,1fr))", gap: 14 }}>
          <KpiCard label="Total Companies"  value={`${totalCompanies}`}               sub="Registered on platform"   icon="🏢" accent="#f59e0b" spark={[32,35,38,39,41,42,44,47]} trend="↑ 3 this month" trendUp />
          <KpiCard label="Monthly Revenue"  value={`₹${(totalRevenue/1000).toFixed(0)}K`} sub="Across all subscriptions" icon="💰" accent="#10b981" spark={[210,230,218,245,228,261,274,285]} trend="↑ 7.4% MoM" trendUp />
          <KpiCard label="Total Users"      value={`${totalUsers.toLocaleString()}`}  sub="Employees on platform"    icon="👥" accent="#8b5cf6" ring={Math.round(4812/6000*100)} trend="↑ 124 this week" trendUp />
          <KpiCard label="Live Sessions"    value={`${activeSessions}`}               sub="Right now"                icon="⚡" accent="#3b82f6" spark={[280,295,310,298,320,308,315,312]} trend="↑ 18 vs 1h ago" trendUp />
        </div>
        ...
        {/* ── Charts row ── */}
        <div className="ad-a4" style={{ display: "grid", gridTemplateColumns: "1fr 0.55fr 0.45fr", gap: 14 }}>
          ...
          <GCard accent={tk.sec} style={{ padding: "18px 20px" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 18 }}>
              <div style={{ position: "relative" }}>
                <Donut segs={planDistribution} size={100} />
                <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                  <div style={{ fontSize: 18, fontWeight: 800, color: tk.acc }}>47</div>
                  <div style={{ fontSize: 9, color: tk.textMuted, fontWeight: 600 }}>TOTAL</div>
                </div>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
                {planDistribution.map((s, i) => (
        ...
        <div className="ad-a5">
          <div style={{ display: "flex", gap: 4, marginBottom: 16, borderBottom: `1px solid ${tk.divider}` }}>
            {(["companies", "system", "logs"] as const).map(tab => (
              <button key={tab} className="ad-tab"
                  ...
                  {(["All", "Enterprise", "Pro", "Starter"] as const).map(p => (
                    <button key={p} onClick={() => setPlanFilter(p)}
                      style={{
                        ...
                        fontFamily: "inherit", cursor: "pointer", transition: "all 0.15s", border: "1px solid",
          ...
          {activeTab === "system" && (
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              {sysMetrics.map((m, i) => {
                const pct = Math.round((m.value / (m.unit === "ms" ? 1000 : m.unit === "MB/s" ? 100 : 100)) * 100);
          ...
          {activeTab === "logs" && (
            <GCard style={{ padding: "18px 20px" }}>
              ...
              <div>
                {[...activities, ...activities.map(a => ({ ...a, id: a.id + 100, time: "Yesterday" }))].map((act, i, arr) => (
                  <div key={act.id} style={{ display: "flex", gap: 12, padding: "10px 0", borderBottom: i < arr.length - 1 ? `1px solid ${tk.divider}` : "none", animation: `ad-up .35s ease both ${.04*i}s` }}>
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                      ...
                      {i < arr.length - 1 && <div style={{ width: 1, flex: 1, background: tk.divider, marginTop: 4 }} />}
                    </div>
        ...
        {/* ── Bottom row: Activity + Top companies ── */}
        <div className="ad-a7" style={{ display: "grid", gridTemplateColumns: "1fr 0.8fr", gap: 14 }}>

          {/* Live activity */}
          <GCard accent={tk.acc} style={{ padding: "18px 20px" }}>
            <CH title="⚡ Live Admin Activity" />
            {activities.map((act, i) => (
              <div key={act.id} style={{ display: "flex", gap: 12, padding: "8px 0", borderBottom: i < activities.length - 1 ? `1px solid ${tk.divider}` : "none", animation: `ad-up .4s ease both ${.04*i}s` }}>
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                  ...
                  {i < activities.length - 1 && <div style={{ width: 1, flex: 1, background: tk.divider, marginTop: 3 }} />}
                </div>
                ...
                <div key={co.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "7px 0", borderBottom: i < 5 ? `1px solid ${tk.divider}` : "none", animation: `ad-up .35s ease both ${.05*i}s` }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ height: 3, background: tk.barTrack, borderRadius: 99, overflow: "hidden", marginTop: 4 }}>
                      <div style={{ height: "100%", width: `${(co.revenue / maxRev) * 100}%`, background: `linear-gradient(90deg,${tk.acc},${tk.sec})`, borderRadius: 99 }} />
                    </div>
                  </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              ...
              <Field label="Industry">
                <select className="ad-inp" value={formData.industry} onChange={e => setFormData(p => ({ ...p, industry: e.target.value }))} style={{ ...is, cursor: "pointer", color: formData.industry ? tk.text : tk.textMuted }}>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
              ...
              <button className="ad-btn" onClick={addCompany}
                style={{ flex: 2, padding: "11px", borderRadius: 10, background: `linear-gradient(135deg,${tk.acc},${tk.accDark})`, border: "none", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", boxShadow: `0 0 18px ${tk.acc}40` }}>
                Register Company
              </button>
            </div>
          </div>
        ...
        <Modal onClose={() => setShowSuspendId(null)} title={`${companies.find(c => c.id === showSuspendId)?.status === "suspended" ? "✅ Restore" : "⚠️ Suspend"} Company`} accent={tk.warn}>
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <button className="ad-btn" onClick={() => toggleSuspend(showSuspendId)}
        ...
        <Modal onClose={() => setShowDeleteId(null)} title="🗑️ Delete Company" accent="#f43f5e">
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <button className="ad-btn" onClick={() => deleteCompany(showDeleteId)}
